...
void Calculate(std::vector<Matrix *> keys, std::vector<Matrix *> values,
  ...
  for (size_t i = 0; i < keys.size(); ++i) {
    ...
    // Step 1: Concatenate first r keys into K (shape [r, d]) in HBM
    Matrix* K = matrix_memory_allocator.Allocate("K_" + std::to_string(i));
    if (r == 1) {
        gpu_sim.Copy(keys[0], K, kInGpuHbm);
    } else {
        Matrix* temp = matrix_memory_allocator.Allocate("temp_k_" + std::to_string(i));
    ...
    // Step 2: Concatenate first r values into V (shape [r, d]) in HBM
    ...
    // Step 3: Transpose K to get K^T (shape [d, r]) in HBM
    gpu_sim.Transpose(K, kInGpuHbm);

    // Step 4: Move Q, K (now K^T), V to SRAM for computation
    ...
    if (r == 1) {
        ...
        Matrix* temp_soft = matrix_memory_allocator.Allocate("temp_soft_" + std::to_string(i));
